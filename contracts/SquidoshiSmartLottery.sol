...
contract SquidoshiSmartLottery is ISmartLottery, LPSwapSupport, LockableFunction, AuthorizedListExt, UserInfoManager {
    ...
    IERC20Detailed public lotteryToken;
    IERC20 public squidoshiToken;
    ...
    constructor(
        ...
        squidoshiToken = IERC20(payable(squidoshi));
        minSpendAmount = 0;
        ...
        if (_rewardsToken == address(0)) {
            ...
            lotteryToken = IERC20Detailed(payable(_rewardsToken));
    ...
    receive() external payable {
        if (!inSwap) swap(BUSD.balanceOf(address(this)));
    }

    function deposit(uint256 _amount) external payable virtual override onlyOwner {
    ...
    function swap(uint256 _amount) internal lockTheSwap {
        if (rewardType == RewardType.TOKEN) {
            swapCurrencyForTokensAdv(address(lotteryToken), _amount, address(this));
        }
    }
    ...
    function setJackpotToToken(address _tokenAddress, bool andSwap) external virtual override authorized {
        require(
            rewardType != RewardType.TOKEN || _tokenAddress != address(lotteryToken)
        );
    ...
    function resetToToken(address _tokenAddress, bool andSwap) private lockTheSwap {
        ...
        lotteryToken = IERC20Detailed(payable(_tokenAddress));
    ...
    function isEligibleForJackpot(address participant) external view returns (bool) {
        if (
            ...
            hodlerInfo[participant].tokenBalance < eligibilityCriteria.minSquidoshiBalance
        ) return false;
    ...
    function _isEligibleForJackpot(address participant) private view returns (bool) {
        ...
        if (
            hodlerInfo[participant].lastTransfer < block.timestamp.sub(eligibilityCriteria.timeSinceLastTransfer) &&
    ...
    function pseudoRand() private view returns (uint256) {
        ...
        uint256 someValue = uint256(
            keccak256(abi.encodePacked(nonce, msg.sender, gasleft(), block.timestamp, draw, jackpotParticipants.at(0)))
        );
    ...
    function excludeFromJackpot(address user, bool shouldExclude) public override authorized {
        if (
            ...
            !shouldExclude &&
            hodlerInfo[user].tokenBalance >= eligibilityCriteria.minSquidoshiBalance
        ) jackpotParticipants.add(user);
        if (!isExcludedFromJackpot[user] && shouldExclude) jackpotParticipants.remove(user);
    ...
    function setMaxAttempts(uint256 attemptsToFindWinner) external override authorized {
        require(
            attemptsToFindWinner > 0 && attemptsToFindWinner != maxAttemptsToFindWinner
        );
